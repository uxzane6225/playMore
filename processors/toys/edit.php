...
try {
    ...
    if (!$result) {
        $_SESSION['error'] = "Toy doesn't exist!";
        header("Location: ../../pages/toys/edit.php?tid={$_SESSION['tid']}");
        exit;
    }
    ...
    if (!empty($file['name'])) {
        ...
        if (move_uploaded_file($file['tmp_name'], $targetPath)) {
            $stmt = $pdo->prepare("UPDATE toys SET name = :name, description = :description, min_age = :min_age, max_age = :max_age, price = :price, stock = :stock, imagepath = :imagepath, aid = :aid, tcid = :tcid, ttid = :ttid, bid = :bid WHERE tid = :tid");
            $stmt->execute([
                ":tid" => $_SESSION['tid'],
                ":name" => $name,
        ...
        $stmt = $pdo->prepare("UPDATE toys SET name = :name, description = :description, min_age = :min_age, max_age = :max_age, price = :price, stock = :stock, aid = :aid, tcid = :tcid, ttid = :ttid, bid = :bid WHERE tid = :tid");
        $stmt->execute([
            ":tid" => $_SESSION['tid'],
            ":name" => $name,
            ":description" => $description, 
